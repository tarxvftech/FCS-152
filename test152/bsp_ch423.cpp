...
void CH423_Init() {
    bsp_IIC_Init();                            //Initialize I2C's IO port - As described in "bsp_IIC.h"
    //Set low
...
void CH423_Write(unsigned short cmd) {         //Write command
    IIC_Start();                               //Start the I2C bus (Send the begin signal of I2C - As described in "bsp_IIC.h")
    IIC_Send_Byte(((unsigned char)(cmd>>7) & CH423_I2C_MASK) | CH423_I2C_ADDR1);
    IIC_Wait_Ack();
    IIC_Send_Byte((unsigned char) cmd);        //Send data (I2C send a byte)
    IIC_Wait_Ack();
    IIC_Stop();                                //Stop the I2C bus (Send the stop signal of I2C - As described in "bsp_IIC.h") 
}
